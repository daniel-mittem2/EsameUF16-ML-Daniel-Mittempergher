...
Eseguire i task nell'ordine indicato. Ogni task deve essere committato
separatamente prima di iniziare il successivo.
...
- [ ] Creare `requirements-dev.txt` con: `pytest`, `pytest-cov`
...
- [ ] Verificare che un metodo non consentito su `/health` risponda 405 con corpo JSON
...
## T-04 — Moduli di supporto (`errors.py`, `pagination.py`, `models.py`)

- [ ] `errors.py`: costanti codici errore (`VALIDATION_ERROR`, `NOT_FOUND`,
  `EMAIL_ALREADY_EXISTS`, `MALFORMED_JSON`, `METHOD_NOT_ALLOWED`),
  `make_error_response(code, message, details={}, status=None)` che produce
...
- [ ] Per ogni rotta: validare il body con `validators.py`, delegare al
...
- [ ] Verificare `PUT /api/v1/users/<non-existent-route>` → 405 con corpo JSON
  ...
  **solo** `user`:
  ...
  services:
    user:
      ...
      health_path: /health
  ```
- [ ] Installare le dipendenze del servizio in un venv o nell'ambiente attivo:
  `py -3.12 -m pip install flask requests pytest pytest-cov`
