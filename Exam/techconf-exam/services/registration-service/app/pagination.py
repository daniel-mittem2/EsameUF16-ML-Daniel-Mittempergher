"""Pagination parsing and slicing helpers for the registration-service.
...
length (post-filter, pre-pagination) (REQ-REG-F05-AC6).
"""
...
def parse_pagination_params(args) -> Tuple[int, int]:
    ...
    ``PaginationError`` on any violation (REQ-REG-F05-AC2/AC3).
    """
...
def paginate(items: List[dict], page: int, page_size: int) -> dict:
    """Return a RegistrationPage dict for ``items`` sliced by ``page``/``page_size``.

    ``total`` is the length of ``items`` before slicing (post-filter,
    pre-pagination) (REQ-REG-F05-AC6). A page beyond the last returns an empty
