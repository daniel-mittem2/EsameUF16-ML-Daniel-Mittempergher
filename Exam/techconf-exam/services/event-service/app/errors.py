"""Error codes and standard error response builder for the event-service.

All error responses across the service conform to the ``Error`` schema of the
contract: ``{"error": {"code": "UPPER_SNAKE", "message": "...", "details": {}}}``.
The ``details`` field is always a JSON object, never ``null`` (REQ-EVT-F12-AC4).
...
# --- Error code constants (UPPER_SNAKE_CASE, REQ-EVT-F12-AC2) ---------------
...
# Mapping error code -> default HTTP status code (REQ-EVT-F12-AC3).
...
def make_error_response(
    ...
    (REQ-EVT-F12-AC4). When ``status`` is not provided, it is derived from
