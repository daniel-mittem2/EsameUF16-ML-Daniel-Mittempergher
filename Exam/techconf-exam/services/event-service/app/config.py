"""Configuration loading for the event-service (REQ-EVT-F13).
...
without any environment variables set (REQ-EVT-F13-AC5).
"""
...
# Module-level sentinel so that ``load_config(port=5002)`` bypasses the env
...
def load_config(port=_SENTINEL) -> Config:
    ...
    integer (REQ-EVT-F13-AC1). ``STORAGE_BACKEND`` defaults to ``"memory"``
    and ``DATA_DIR`` defaults to ``Path("./data")`` (REQ-EVT-F13-AC3).
    ``USER_SERVICE_URL`` defaults to ``"http://localhost:5001"``
    (REQ-EVT-F13-AC2).
